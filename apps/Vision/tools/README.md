...
(ONNX) into `vision/Models/ArcFace.mlpackage`, ready for `ArcFaceEmbedder.swift`
...
```bash
cd /Users/jonathanzhou/Documents/vision
python3 -m venv .venv
...
2. **Verifies numerical parity**: runs the same random input through the
   ...
   do not wire it in.
3. Saves `vision/Models/ArcFace.mlpackage`.

Expect it to take a few minutes, mostly the one-time package installs.

## After it succeeds

Add `vision/Models/ArcFace.mlpackage` to the Xcode project if it doesn't
show up automatically (the `vision/` folder is a file-system-synchronized
group, so it should auto-appear — if not, drag it into Xcode and make sure
"Copy items if needed" + the `vision` target are checked).
