...
final class CameraManager: NSObject {
    ...
    private let sessionQueue = DispatchQueue(label: "com.sharan.vision.camera.session")
        ...
        guard permission == .granted else {
            errorMessage = "Camera access not granted (status: \(describe(status))). " +
                (status == .restricted
                    ...
                    : "Enable it in System Settings > Privacy & Security > Camera. If vision isn't listed there, quit the app, run `tccutil reset Camera com.sharan.vision` in Terminal, then relaunch so macOS asks again.")
            return
        }
