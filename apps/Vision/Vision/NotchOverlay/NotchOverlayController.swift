...
final class NotchOverlayController {
    ...
    /// Snapshotted from `VisionSettings` when a cycle begins rather than read live,
    ...
    private var scanTimeoutDuration: Duration {
        .seconds(VisionSettings.shared.faceDetectionSeconds)
    }
    ...
    func beginScanning() {
        ...
        geometry = windowController.currentGeometry
        activeUnlockStyle = VisionSettings.shared.effectiveUnlockAnimationStyle
        content = .scan(.idle)
        phase = .scanning
        updateInteractivity()
    ...
    func present(styleOverride: UnlockAnimationStyle? = nil, onRetry: (() -> Void)? = nil) {
        ...
        scanTimeoutTask?.cancel(); scanTimeoutTask = nil
        geometry = windowController.currentGeometry
        activeUnlockStyle = styleOverride ?? VisionSettings.shared.effectiveUnlockAnimationStyle
    ...
    func activate() {
        ...
        guard VisionSettings.shared.retryOnHover else { return }
        switch phase {
        case .closed, .failure:
