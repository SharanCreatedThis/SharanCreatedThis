    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                ...
                .padding(.vertical, 6)
                .background(isDestructive ? Color.red.opacity(0.85) : VisionTheme.accent)
                .clipShape(Capsule())
        }
        .buttonStyle(.plain)
        .disabled(!isEnabled)
            ...
            VStack(spacing: 8) {
                preview()
                    ...
                    .background {
                        ...
                        if isSelected {
                            VisionTheme.accent.opacity(SettingsMetrics.optionPreviewSelectedTintOpacity)
                        }
                    }
...
final class WindowConfiguringView: NSView {
    ...
    private static func configure(_ window: NSWindow) {
        ...
        let toolbar = NSToolbar(identifier: "VisionSettingsToolbar")
        ...
        // Unconstrain min/max first so AppKit never clamps setFrame to an older minSize
