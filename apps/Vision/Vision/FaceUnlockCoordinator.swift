...
final class FaceUnlockCoordinator {
    ...
    /// Persisted via VisionSettings. Setting to false cancels any in-flight scan and disarms the overlay immediately.
    var isEnabled: Bool {
        didSet {
            VisionSettings.shared.isFaceUnlockEnabled = isEnabled
    ...
    var matchThreshold: Float {
        didSet { VisionSettings.shared.matchThreshold = matchThreshold }
    }
    ...
    private var scanWindowDuration: TimeInterval {
        TimeInterval(VisionSettings.shared.faceDetectionSeconds)
    }
    ...
    private var showsUI: Bool { VisionSettings.shared.showUnlockAnimation }
    ...
    init(pocController: POCController) {
        ...
        self.isEnabled = VisionSettings.shared.isFaceUnlockEnabled
        self.matchThreshold = VisionSettings.shared.matchThreshold
        spaceKeyMonitor.onSpaceKeyDown = { [weak self] in self?.handleSpaceKeyPress() }
        observeLockAndWakeEvents()
        observeVaultUnlockRequests()
    }
    ...
    private func evaluateTrigger() {
        ...
        let shouldAutoScan = VisionSettings.shared.unlockTriggers.contains(signal)
    ...
    private func updateSpaceMonitor() {
        let shouldListen = isEnabled
            && VisionSettings.shared.unlockTriggers.contains(.onSpace)
    ...
    private func scheduleAutoRetryIfEnabled(after delay: Duration) {
        guard VisionSettings.shared.autoRetryOnce, !hasAutoRetriedForCurrentLock else { return }
    ...
    private func observeScanWindow(deadline: Date, requireOverlayScanning: Bool) async -> ScanOutcome {
        let livenessEnabled = VisionSettings.shared.livenessChecksEnabled
        let liveness = LivenessAnalyzer()
        liveness.modeProvider = { VisionSettings.shared.livenessMode }
    ...
    private func observeVaultUnlockRequests() {
        NotificationCenter.default.addObserver(
            forName: Notification.Name("VisionVaultRequestAppUnlock"),
            object: nil,
            queue: .main
            ...
            guard let self,
                  ...
                  let bundleID = userInfo["bundleID"] as? String else { return }
            let appName = (userInfo["appName"] as? String) ?? app.localizedName ?? "App"

            Task { @MainActor in
                self.performVaultUnlockScan(app: app, bundleID: bundleID, appName: appName)
            }
        }
    }

    public func performVaultUnlockScan(app: NSRunningApplication, bundleID: String, appName: String) {
        guard VisionSettings.shared.isVaultEnabled else {
            VaultAppLockManager.shared.grantUnlock(forBundleID: bundleID, app: app)
            return
        }
        ...
        let generation = scanGeneration
        scanTask = Task { [weak self] in
            await self?.runVaultScanCycle(generation: generation, app: app, bundleID: bundleID, appName: appName)
        }
    }

    private func runVaultScanCycle(generation: Int, app: NSRunningApplication, bundleID: String, appName: String) async {
        await camera.start()
        guard generation == scanGeneration else { return }
        ...
        NotchOverlayController.shared.beginScanning()
        statusMessage = "Authenticating for \(appName)…"

        let maxRetries = 5
        let retryInterval: TimeInterval = 2.0
        var matched = false

        // Initial scan attempt + up to 5 retries
        for attempt in 0...maxRetries {
            if attempt > 0 {
                // Update retry state on VisionVaultManager so shield overlay can show progress
                VisionVaultManager.shared.faceRetryCount = attempt
                VisionVaultManager.shared.retryStatusMessage = "Retry \(attempt) of \(maxRetries)…"
                statusMessage = "Face not recognized — retry \(attempt) of \(maxRetries)…"

                // Wait 2 seconds before retrying
                try? await Task.sleep(nanoseconds: UInt64(retryInterval * 1_000_000_000))
                guard generation == scanGeneration, !Task.isCancelled else { break }

                // Re-arm the notch overlay for the retry scan
                NotchOverlayController.shared.beginScanning()
            }

            let deadline = Date().addingTimeInterval(scanWindowDuration)
            ...
            while Date() < deadline, !Task.isCancelled {
                ...
                if let result = try? pipeline.recognize(in: frame.image) {
                    let scored = pipeline.score(result.embedding, against: FaceEnrollmentStore.shared.activeIdentities)
                    if pipeline.bestMatch(in: scored, threshold: matchThreshold) != nil {
                        matched = true
                        break
                    }
                }
            ...
            // Show failure flash on the notch overlay between retries
        ...
        if matched {
            NotchOverlayController.shared.finish(success: true)
            VaultAppLockManager.shared.grantUnlock(forBundleID: bundleID, app: app)
            VaultAuditLogger.shared.logAccess(targetName: appName, authMode: VaultAuthMode.faceOnly.rawValue, success: true)
            VisionVaultManager.shared.retryStatusMessage = "Face recognized ✓"
            VisionVaultManager.shared.faceRetryCount = 0
        } else {
            // All retries exhausted — fall back to Touch ID (fingerprint)
            NotchOverlayController.shared.finish(success: false)
            statusMessage = "Face verification failed — use fingerprint"
            VisionVaultManager.shared.retryStatusMessage = "Use fingerprint to unlock"
            VisionVaultManager.shared.isFallingBackToTouchID = true

            VaultAuditLogger.shared.logAccess(
                targetName: appName,
                authMode: VaultAuthMode.faceOnly.rawValue,
                success: false,
                failureReason: "Face verification failed after \(maxRetries) retries — falling back to Touch ID"
            )

            // Present Touch ID / fingerprint prompt
            let context = LAContext()
            let reason = "Face verification failed. Unlock \(appName) with fingerprint."
            ...
            context.evaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, localizedReason: reason) { success, error in
                Task { @MainActor in
                    if success {
                        ...
                        VaultAuditLogger.shared.logAccess(
                            targetName: appName,
                            authMode: "Face Only → Touch ID fallback",
                            success: true
                        )
                        VisionVaultManager.shared.retryStatusMessage = ""
                        VisionVaultManager.shared.isFallingBackToTouchID = false
                        VisionVaultManager.shared.faceRetryCount = 0
                    } else {
                        ...
                        VisionVaultManager.shared.retryStatusMessage = "Authentication failed"
                        VisionVaultManager.shared.isFallingBackToTouchID = false
                        VisionVaultManager.shared.faceRetryCount = 0
                    }
                }
