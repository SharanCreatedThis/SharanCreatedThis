...
final class POCController {
    ...
    func unlockSession() async {
        ...
        do {
            try await Task.detached(priority: .userInitiated) {
                try SecureCredentialManager.unlockSession(reason: "Authenticate to set up or use vision")
            }.value
    ...
    func injectStoredPassword(requireAuthoritativeLock: Bool = false) async {
        guard KeystrokeInjector.isAccessibilityTrusted() else {
            statusMessage = "Accessibility not granted — open System Settings and enable vision."
            return
        }
        ...
        do {
            ...
            VisionAnalytics.shared.recordUnlock()
        } catch {
            VisionAnalytics.shared.recordFailure()
