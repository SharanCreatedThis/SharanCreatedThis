...
struct EnrollmentRingView: View {
    ...
    var body: some View {
        ...
        ZStack {
            ...
            Circle()
                .stroke(VisionTheme.accent, lineWidth: OnboardingMetrics.completionRingWidth)
                .frame(
    ...
    private func color(for index: Int, intensity: Double) -> Color {
        if isLit(index) || isComplete { return VisionTheme.accent }
        return VisionTheme.whiteToAccent(intensity)
    }
