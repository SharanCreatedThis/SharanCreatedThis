...
struct PillButton: View {
    ...
    private var background: Color {
        style == .primary ? VisionTheme.accent : VisionTheme.surface
    }
}
...
struct PermissionRow: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            Circle()
                .fill(granted ? VisionTheme.statusGranted : VisionTheme.statusDenied)
                .frame(width: OnboardingMetrics.statusDotSize, height: OnboardingMetrics.statusDotSize)

            VStack(alignment: .leading, spacing: 1) {
                Text(title)
                    .font(VisionTheme.Font.rowTitle)
                    .foregroundStyle(VisionTheme.textPrimary)
                Text(detail)
                    .font(VisionTheme.Font.rowDetail)
                    .foregroundStyle(VisionTheme.textDetail)
                    .lineLimit(1)
...
struct PillSecureField: View {
    ...
    var body: some View {
        SecureField("", text: $text, prompt: Text(placeholder).foregroundStyle(VisionTheme.placeholder))
            .textFieldStyle(.plain)
            .font(VisionTheme.Font.passwordPlaceholder)
            .foregroundStyle(VisionTheme.textPrimary)
            .focused($isFocused)
            .onSubmit(onSubmit)
...
struct PillTextField: View {
    ...
    var body: some View {
        TextField("", text: $text, prompt: Text(placeholder).foregroundStyle(VisionTheme.placeholder))
            .textFieldStyle(.plain)
            .font(VisionTheme.Font.passwordPlaceholder)
            .foregroundStyle(VisionTheme.textPrimary)
            .focused($isFocused)
