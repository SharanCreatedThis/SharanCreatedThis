...
/// `String`-backed so `VisionSettings.onboardingResumeStep` can persist it directly by name.
...
final class OnboardingController {
    ...
    private(set) var step: OnboardingStep = .intro {
        didSet {
            ...
            if step == .complete {
                VisionSettings.shared.hasCompletedOnboarding = true
                ...
                VisionSettings.shared.hasAcknowledgedSecurityNotice = true
                VisionSettings.shared.onboardingResumeStep = nil
            } else {
                VisionSettings.shared.onboardingResumeStep = step.resumeTarget
            }
        }
    }
    ...
    static func startFlow(resumingAt step: OnboardingStep? = nil, onFirstRunComplete: (() -> Void)? = nil) {
        VisionAnalytics.shared.track(.onboardingStarted, properties: ["source": step == nil ? "initial" : "resumed"])
    ...
    private func refreshPermissions() {
        ...
        let priorState = cameraPermission
        switch AVCaptureDevice.authorizationStatus(for: .video) {
        case .authorized:
            cameraPermission = .granted
            if priorState != .granted {
                VisionAnalytics.shared.track(.cameraPermissionGranted)
            }
    ...
    var cameraSelectionLabel: String {
        if let id = VisionSettings.shared.defaultCameraID,
    ...
    func selectCamera(id: String?) {
        VisionSettings.shared.defaultCameraID = id
    ...
    func applyDisplayPinForCameraSelection() {
        ...
        if isBuiltIn {
            ...
            VisionSettings.shared.preferredDisplayID = builtInScreen.stableDisplayID
            VisionSettings.shared.preferredDisplayName = builtInScreen.localizedName
        } else {
            VisionSettings.shared.preferredDisplayID = nil
            VisionSettings.shared.preferredDisplayName = nil
        }
    }
    ...
    private func commitEnrollment(name: String) throws {
        ...
        VisionAnalytics.shared.track(.faceRegistered, properties: ["samples_count": samples.count])
    }
    ...
    func finish(password: String) async -> Bool {
        ...
        do {
            try await Task.detached(priority: .userInitiated) {
                try SecureCredentialManager.unlockSession(reason: "Set up Vision")
            }.value
