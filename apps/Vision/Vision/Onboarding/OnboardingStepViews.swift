    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            VStack(alignment: .leading, spacing: 4) {
                Text("Vision")
                    .font(VisionTheme.Font.title)
                    .foregroundStyle(VisionTheme.textPrimary)
                Text("Face Unlock for Mac")
                    .font(VisionTheme.Font.button)
                    .foregroundStyle(VisionTheme.textSecondary)
...
private struct VisionLogoView: View {
    var body: some View {
...
struct PermissionsStepView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            PermissionRow(
                title: "Accessibility",
                detail: "Allow Vision to unlock your Mac",
                granted: controller.accessibilityGranted
            ) { controller.grantAccessibility() }

            PermissionRow(
                title: "Camera",
                detail: "Allow Vision to recognize your face",
...
struct SecurityNoticeStepView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Vision is not as secure as Apple's FaceID or TouchID.")
                .font(VisionTheme.Font.title)
                .foregroundStyle(VisionTheme.textPrimary)
            ...
            Text("It uses your Mac's standard webcam and is designed for convenience, not high-security authentication.")
                .font(VisionTheme.Font.passwordCaption)
                .foregroundStyle(VisionTheme.textSecondary)
                .fixedSize(horizontal: false, vertical: true)
                .padding(.leading, 4)
                .padding(.bottom, 10)
...
struct EnrollStepView: View {
    ...
    private var cameraCluster: some View {
        ZStack {
            ...
            if controller.showCheckmark {
                AnimatedCheckmark(color: VisionTheme.accent, lineWidth: 8)
    ...
    private var instructionLabel: some View {
        Text(controller.enrollmentInstruction)
            .font(VisionTheme.Font.instruction)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let error = controller.nameError {
                Text(error)
                    .font(VisionTheme.Font.rowDetail)
                    .foregroundStyle(VisionTheme.statusDenied)
                    .fixedSize(horizontal: false, vertical: true)
            }
...
struct PasswordStepView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Your password is required to unlock your Mac. It is encrypted and securely stored on your device. Vision works entirely offline, so your password never leaves your Mac.")
                .font(VisionTheme.Font.passwordCaption)
                .foregroundStyle(VisionTheme.textSecondary)
                .fixedSize(horizontal: false, vertical: true)
                .padding(.leading, 4)
