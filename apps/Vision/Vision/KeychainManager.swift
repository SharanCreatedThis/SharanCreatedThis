...
enum KeychainManager {
    nonisolated static let service = "com.sharan.vision"
    ...
    nonisolated static func save(account: String, data: Data, accessControl: SecAccessControl? = nil) throws {
        ...
        // If accessControl fails with errSecMissingEntitlement (-34018) (e.g. ad-hoc or local development builds without Apple Developer Keychain entitlement),
        // fallback to standard device-unlocked accessibility so password and session key storage work reliably across all environments.
