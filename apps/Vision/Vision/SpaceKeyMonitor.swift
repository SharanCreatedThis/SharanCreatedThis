...
//
//  vision never appears under Input Monitoring: TCC resolves that gate against Accessibility first, and vision already requires
...
final class SpaceKeyMonitor {
    ...
    static let log = Logger(subsystem: "com.sharan.vision", category: "inputmonitoring")
    ...
    /// True when launched by Xcode's Run button, which makes TCC decisions attribute to Xcode, not vision — test permission
    /// behavior from an independently launched copy instead (`open /path/to/vision.app`, or a build in /Applications).
