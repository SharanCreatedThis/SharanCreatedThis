<h1 align="center">
  <br>
  <a href="https://tryvision.app"><img src="Vision/Assets.xcassets/AppIcon.appiconset/AppIcon-1024.png" alt="Vision" width="150"></a>
  <br>
  Vision
  <br>
</h1>
...
Vision brings the FaceID-like experience of your iPhone to a Mac near you. Unlock your Mac with Vision — no typing, no reaching for the TouchID key. Everything runs on-device using Apple's Vision
...
> ## Vision is not as secure as Apple's FaceID or TouchID
> 
...
> 
> - Vision defeats, with reasonable confidence, a printed photo and a photo on a phone screen (heavy liveness detection must be turned on)
> - Vision does not reliably defeat a video of you
> - macOS has no API that lets a third-party app authorize a login, so Vision unlocks by typing
>   your stored password on the lock screen
> 
> Vision is a convenience feature, not a security upgrade. Only continue if you accept the tradeoff.
...
<a href="https://github.com/jonnyoo/vision/releases/latest/download/Vision.dmg" target="_self"><img width="200" src="https://github.com/user-attachments/assets/cdb8af97-1ee2-4669-b7cb-dcfb56c9dd61" alt="Download for Mac" /></a>

Open the `.dmg` file and drag Vision to `/Applications`, then open it.
...
1. Launch the app and follow the onboarding to enroll your face. Vision guides you through capturing your face, turning your head in nine
...
4. If it's you — and the liveness checks agree you're a real person — Vision types the
...
| **Notch UI** | A closed pill that expands into a scan animation with success and failure states. Hover to retry — or turn animations off entirely and Vision stays invisible. |
...
Vision is designed to keep biometric data and credentials on-device.

### Face data

Vision never stores camera images. During enrollment, each captured face is converted into a **512-dimensional embedding** using an ArcFace-based Core ML model. The original frame is then discarded.
...
The key is only held in memory while an authorized Vision session is active.

### Unlock pipeline

Vision won't type your password simply because a face matches. An unlock requires all of the following:

1. A valid Vision session is authorized.
...
Face recognition, face enrollment, and liveness detection run entirely on-device using Vision and Core ML. Vision does not send face data, camera frames, or credentials to a server.
...
1. Clone repository:
  ```bash
   git clone https://github.com/jonnyoo/vision.git
   cd vision
  ```
2. Open in Xcode:
  ```bash
...
App feedback goes to [tryvision.app/feedback](https://tryvision.app/feedback).
