...
# The version is stated once, in project.yml, and read from there. The mounted
# volume and the image's metadata carry it so that two downloads sitting in the
# same Downloads folder can be told apart, and so a mounted volume says which
# release it is.
#
# The *file* is deliberately not versioned: README links to
# releases/latest/download/Hangly.dmg, which needs the asset's name to be the same
# in every release.
MARKETING_VERSION="$(awk -F'"' '/^    MARKETING_VERSION:/ { print $2 }' "$ROOT/project.yml")"
...
echo "==> Building $APP_NAME $MARKETING_VERSION ($CONFIGURATION)"
xcodebuild \
...
# What the image says it is, independently of its file name. `hdiutil imageinfo`
# reads this back, and so does anything cataloguing a download.
BUILT_VERSION="$(defaults read "$DIST/$APP_NAME.app/Contents/Info" CFBundleShortVersionString)"
BUILT_BUILD="$(defaults read "$DIST/$APP_NAME.app/Contents/Info" CFBundleVersion)"
if [ "$BUILT_VERSION" != "$MARKETING_VERSION" ]; then
  echo "error: the built app says $BUILT_VERSION where project.yml says $MARKETING_VERSION" >&2
  exit 1
fi
echo "    version $BUILT_VERSION ($BUILT_BUILD), volume \"$VOLUME_NAME\""
...
echo "  version $BUILT_VERSION ($BUILT_BUILD)"
