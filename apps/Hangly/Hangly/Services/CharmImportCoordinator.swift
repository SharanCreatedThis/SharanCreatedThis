...
final class CharmImportCoordinator {
    ...
    /// Deletes the charm on the end of the rope, for the menu, which has no other
    /// way of saying which one it means.
    func deleteCurrentCharm() {
        deleteCharm(charmManager.selection)
    }

    /// Deletes one charm after confirmation. Does nothing for a built-in.
    ///
    /// Takes the charm rather than reading the rope, because the Library can be
    /// describing a charm that is not hanging anywhere.
    func deleteCharm(_ charm: CharmID) {
        guard case .custom(let id) = charm,
