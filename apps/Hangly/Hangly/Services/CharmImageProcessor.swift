...
enum CharmImageProcessor {
    ...
    ///
    /// Mass follows how much shape there is: a solid form is heavier than a wispy
    /// one, which is both intuitive and what keeps very different imports swinging
    /// believably. Measured at a fixed framing, so re-framing the subject does not
    /// quietly re-weigh it — Fill is not a second weight control.
    ///
    /// The knot inset does follow the framing, and has to: it is where the cord
    /// stops, and the cord has to stop on the artwork. A knot that ignored the fill
    /// would hang the charm from a point above its own picture.
    static func analyze(_ square: CGImage) throws -> (metrics: CharmMetrics, palette: CharmPalette) {
        ...
        // How the subject was framed, read back off the square: `fitToSquare` scales
        // the subject until its longest side is that fraction of the side.
        let longestSide = Double(max(bounds.width, bounds.height))
        let framing = (longestSide / Double(bitmap.width)).clamped(to: 0.3...1)

        // Coverage as it would have been had the subject been framed the shipped
        // way. Coverage alone is a fine measure of how much shape there is — a thin
        // bar covers little of its square however it is framed — but it also moves
        // when the subject is merely re-framed, and framing is not weight. Dividing
        // out the framing and multiplying the shipped one back in keeps the first
        // property and drops the second; at the shipped fill the two are the same
        // number, so nothing about an ordinary import changes.
        let density = bitmap.coverage(threshold: opaqueThreshold)
            * (subjectFill * subjectFill) / (framing * framing)
        let mass = (2.0 + (3.2 * density)).clamped(to: 2.0...4.5)
