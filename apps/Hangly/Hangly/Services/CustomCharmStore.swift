...
final class CustomCharmStore {
    ...
    /// Forgets the decoded bitmap of every import except the ones named.
    ///
    /// Browsing the Library, or Create's "Made here" strip, decodes every import
    /// there has ever been. Nothing evicts them afterwards — the cache has no limit,
    /// because until Customize existed nothing loaded more than the one on the rope.
    /// - Parameter keeping: The imports currently on the rope, which are drawn from
    ///   these bitmaps on every frame and must not be dropped.
    /// - Returns: How many were forgotten.
    @discardableResult
    func purgeBitmaps(keeping: Set<UUID>) -> Int {
        let doomed = bitmapCache.keys.filter { !keeping.contains($0) }
        for id in doomed { bitmapCache[id] = nil }
        return doomed.count
    }

    /// The identity of every bitmap still held, so a cache keyed on these can drop
    /// the entries whose source has gone.
    var retainedBitmapIdentities: Set<ObjectIdentifier> {
        Set(bitmapCache.values.map(ObjectIdentifier.init))
    }
