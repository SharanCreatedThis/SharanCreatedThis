...
enum CharmRenderer {
    ...
    ///   - mood: What the weather is doing to it. `.clear` draws the charm exactly
    ///     as it was drawn before weather existed, down to the cache entry it uses.
    static func draw(
        ...
        let palette = mood.applied(to: palette)
        ...
        if let vector = artwork.vector {
            drawVector(
                ...
                placement: VectorPlacement(
                    ...
                    opacity: opacity,
                    rotation: rotation,
                    tint: VectorTint.make(palette: nil, mood: mood)
                ),
                into: &context
            )
            return
        ...
        if let bitmap = artwork.bitmap {
            // An imported charm is one image with no vector behind it, so there is
            // nothing to re-render it from — it is weathered once and kept, keyed by
            // the image and the mood, which for one charm in one sky is one entry.
            let weathered = WeatheredImageCache.shared.image(for: bitmap, mood: mood)
            drawBitmap(weathered, into: &context, rect: rect, opacity: opacity, highlight: true)
            return
        }
...
/// Imported charms, as the weather leaves them.
///
/// A vector charm is re-rendered from its source for every size it is drawn at, and
/// `VectorImage` caches the results; an imported one is a single bitmap with no
/// source to go back to, so its weathered copy is kept here instead. One charm in
/// one sky is one entry, and the whole cache holds a handful — a stack of three
/// imported charms during a weather change is the worst it ever sees.
/// `@unchecked Sendable` for the same reason `VectorImage` is: every access goes
/// through one lock, and a `CGImage` is immutable once made. The renderer draws from
/// whichever thread SwiftUI hands it, so a main-actor cache would be unreachable.
final class WeatheredImageCache: @unchecked Sendable {
    static let shared = WeatheredImageCache()
    ...
    /// Enough for three charms mid-change, and small enough that forgetting one
    /// costs a single pass over an image that is a few hundred pixels a side.
    private static let limit = 8

    private var cache: [Key: CGImage] = [:]
    private var order: [Key] = []

    func image(for image: CGImage, mood: WeatherMood) -> CGImage {
        guard !mood.isNeutral else { return image }
        let key = Key(image: ObjectIdentifier(image), mood: mood)
        ...
        // A failed pass falls back to the artwork: a charm that missed the weather
        // is a blemish, a charm that failed to draw is a bug.
        let weathered = RGBABitmap.weathered(image, mood: mood) ?? image
    ...
    /// Forgets every weathered copy whose source bitmap is no longer held.
    ///
    /// Keyed on the identity of the source, so once `CustomCharmStore` has let a
    /// bitmap go, its weathered copy here is unreachable and pure waste — and the
    /// identity could in principle be handed to a different image at the same
    /// address, which would show as the wrong charm in the wrong sky.
    /// - Returns: How many were forgotten.
    @discardableResult
    func purge(retaining sources: Set<ObjectIdentifier>) -> Int {
        lock.lock()
        defer { lock.unlock() }
        let doomed = cache.keys.filter { !sources.contains($0.image) }
        guard !doomed.isEmpty else { return 0 }
        for key in doomed { cache[key] = nil }
        order.removeAll { !sources.contains($0.image) }
        return doomed.count
    }
}
