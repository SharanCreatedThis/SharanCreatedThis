...
extension CharmRenderer {
    ...
    /// bitmap.
    /// - Parameter tint: The rope style's metal, or `nil` to keep the artwork's own
    ///   colours. A bead is part of the cord assembly, so it follows the cord.
    ...
    struct VectorPlacement {
        ...
        /// A recolour applied to the rasterised artwork, cached with it.
        var tint: VectorTint?
    }
    ...
    static func drawVector(
        ...
        // Set to `.overlay` by the overlay's root view and left at `.interface`
        // everywhere else, so a Library card's bitmaps can be told from the rope's.
        let usage = context.environment.artworkUsage
        let side = radius * 2
