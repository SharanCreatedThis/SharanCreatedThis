...
final class CharmStudioViewModel {
    ...
    /// Lets go of the workspace when there is nothing unfinished in it.
    ///
    /// Create holds three full-resolution images of whatever was last opened — the
    /// source, the isolated subject and the draft — and this view model belongs to
    /// the app rather than to the window, so they outlive the window by the rest of
    /// the run. Closing Customize is a good moment to give them back.
    ///
    /// It is not a good moment to throw away somebody's work, so an image that has
    /// been opened and not yet saved is kept. Coming back to Create finds it where
    /// it was left.
    /// - Returns: Whether anything was discarded.
    @discardableResult
    func discardFinishedWork() -> Bool {
        guard hasImage, stage == .saved else { return false }
        clear()
        Logger.overlay.diagnostic("Studio workspace released; the charm was already saved.")
        return true
    }
