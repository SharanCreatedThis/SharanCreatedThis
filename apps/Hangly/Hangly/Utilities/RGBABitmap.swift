...
struct RGBABitmap {
    ...
    let width: Int
    let height: Int
    /// The pixels themselves. Written here and by the colour pass in
    /// `RGBABitmap+Weather`; internal rather than `private(set)` only because Swift
    /// cannot say "private to this type across its own files".
    var pixels: [UInt8]
    ...
    /// Recolours an image through a luminance ramp, keeping its alpha and its shading.
    ///
    /// Used to put a charm's beads on the cord they are actually hanging from: the
    /// artwork draws them gold, and a silver chain or a neon filament wants them in
    /// its own metal. Replacing the colour outright would flatten them into
    /// silhouettes, so each pixel keeps its brightness and only its hue is exchanged:
    /// a bead's shadowed side maps to the palette's `deep`, its lit side to `light`,
    /// and everything between to `primary`. What was a shaded sphere stays one.
    ///
    /// A per-pixel loop over a bitmap of at most a few hundred pixels a side, run
    /// once per size per style and cached by `VectorImage` — never per frame.
    /// - Parameters:
    ///   - palette: The colours to map into.
    ///   - strength: How far to move from the original colour, zero to one.
    static func tinted(_ image: CGImage, palette: CharmPalette, strength: Double) -> CGImage? {
        let amount = strength.clamped(to: 0...1)
        guard amount > 0, var bitmap = RGBABitmap(image: image) else { return nil }

        for index in stride(from: 0, to: bitmap.pixels.count, by: bytesPerPixel) {
            let alpha = Double(bitmap.pixels[index + 3])
            guard alpha > 0 else { continue }

            // Pixels are premultiplied, so undo that before judging brightness;
            // a half-transparent white is white, not grey.
            let red = Double(bitmap.pixels[index]) / alpha
            let green = Double(bitmap.pixels[index + 1]) / alpha
            let blue = Double(bitmap.pixels[index + 2]) / alpha
            let luminance = ((red * 0.2126) + (green * 0.7152) + (blue * 0.0722)).clamped(to: 0...1)

            let ramp = luminance < 0.5
                ? CharmColor.interpolate(from: palette.deep, to: palette.primary, progress: luminance * 2)
                : CharmColor.interpolate(from: palette.primary, to: palette.light, progress: (luminance - 0.5) * 2)

            func blend(_ original: Double, _ target: Double) -> UInt8 {
                let mixed = original + ((target - original) * amount)
                return UInt8((mixed * alpha).rounded().clamped(to: 0...255))
            }

            bitmap.pixels[index] = blend(red, ramp.red)
            bitmap.pixels[index + 1] = blend(green, ramp.green)
            bitmap.pixels[index + 2] = blend(blue, ramp.blue)
        }
