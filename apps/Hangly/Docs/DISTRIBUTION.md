...
> **This is the 1.0 report.** Its analysis of Gatekeeper, notarisation and what the
> bundle asks the system for is unchanged and still applies. Its numbers do not:
> 2.0 is `MARKETING_VERSION 2.0 (2)`, the app is 9.0 MB and the image 7.2 MB, and
> the suite is 354 tests. See [`RC2-Audit.md`](RC2-Audit.md) for the current
> figures, and [Cutting a release](#10-cutting-a-release) below for the steps.
...
| Network | None unless weather is enabled, which it is not by default. With it on: one Open-Meteo request every thirty minutes, no key and no account |
...
The version is stated once, in `project.yml`, and everything else reads it: the
bundle through `$(MARKETING_VERSION)` in `Info.plist`, and the disk image's volume
name through `Scripts/build-dmg.sh`, which fails the build if the app it just
compiled disagrees with it.

The tag is the version with a `v` and all three parts, matching `v1.0.0` and the
links at the foot of `CHANGELOG.md` — `2.0` in the bundle and `2.0.0` in the tag is
a wart, and the note in `RC2-Audit.md` §6 says what to do about it.

```sh
# 1. The version, if it is changing.
#    project.yml → MARKETING_VERSION, CURRENT_PROJECT_VERSION
xcodegen generate

# 2. Everything must be green before anything is tagged.
xcodebuild -project Hangly.xcodeproj -scheme Hangly -configuration Debug test
swiftlint --strict

# 3. The release notes, the changelog and the archived copy.
#    - RELEASE_NOTES.md is what goes in the GitHub release body
#    - CHANGELOG.md moves [Unreleased] to [x.y.z] — DATE and adds the link
#    - Docs/release-notes/vx.y.z.md is a copy of RELEASE_NOTES.md at that version

# 4. The image.
./Scripts/build-dmg.sh          # → dist/Hangly.app and dist/Hangly.dmg

# 5. The tag, annotated, on the commit that was built.
git tag -a v2.0.0 -m "Hangly 2.0"
git push origin main --follow-tags

# 6. The GitHub release, with the image attached under its plain name —
#    README links to releases/latest/download/Hangly.dmg, which needs the asset
#    to be called exactly that in every release.
gh release create v2.0.0 dist/Hangly.dmg \
  --title "Hangly 2.0" \
  --notes-file RELEASE_NOTES.md
```

Notarisation, when there is a Developer ID to do it with, goes between steps 4
and 6 — see §9.
