...
Only if you turn on weather, which is off until you do. With it on, Hangly asks
[Open-Meteo](https://open-meteo.com) for one weather code every thirty minutes and
nothing else — no account, no API key, no identifier, no analytics. With it off,
no request is made, no connection is opened and no timer runs.

### How does weather know where I am?

It does not, and it never asks macOS. It uses the city named by your system time
zone — `Europe/Lisbon` is Lisbon — or one you type in **Settings → General**. The
coordinates that city resolves to are rounded to two decimal places, roughly a
kilometre, before anything is sent or stored. There is no location permission
prompt because there is no location permission being used.

### What happens to weather when I am offline?

The last reading is kept and carries on being worn. Hangly tries again in five
minutes rather than the usual thirty, and if a reading gets more than six hours old
it is dropped and the charm goes back to its own colours. A Mac that has never been
online with weather on simply looks like a Mac without weather.

### What does it store, and where?

Two things, both plain files on your Mac:

- **Settings** in `~/Library/Preferences/com.hangly.Hangly.plist`, which is also
  where the last weather reading is cached
