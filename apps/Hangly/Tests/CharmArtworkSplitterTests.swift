...
struct CharmArtworkSplitterTests {
    ...
    func bodyReachesTheBottom() throws {
        for entry in CollectionCharmCatalog.entries {
            ...
            // Where the cord meets the body, as the body's height over its longest
            // side. One for anything at least as tall as it is wide, which every
            // hand-drawn charm is; less for a wide one, which is how the cord ends
            // up meeting a spread of wings above their middle rather than at their
            // top. The DC emblems were the first charms in the app wider than they
            // are tall, and this assertion used to read `== 1`.
            let longest = max(regions.body.width, regions.body.height)
            #expect(regions.knotInset > 0, "\(entry.kind) has no knot")
            #expect(regions.knotInset <= 1, "\(entry.kind) knot inset out of range")
            #expect(abs(regions.knotInset - (regions.body.height / longest)) < 1e-9)
            if regions.body.height >= regions.body.width {
                #expect(regions.knotInset == 1, "\(entry.kind) is tall, so its knot is at the top")
            }
        }
    }
