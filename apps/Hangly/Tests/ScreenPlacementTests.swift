...
struct ScreenPlacementTests {
    ...
    @Test("An extreme offset is clamped so the rope stays on screen")
    func extremeOffsetsAreClamped() {
        ...
        // What is kept on screen is the point the charm hangs from, not the whole
        // window. The window is mostly transparent room for the charm to swing in,
        // and insisting all of it stay on screen stopped the charm half a window
        // short of either edge — the dead zones the position control used to have.
        #expect(farRight.midX == bounds.maxX)
        #expect(farLeft.midX == bounds.minX)
        #expect(farDown.maxY == bounds.minY)

        // The window itself is allowed to overhang, and does.
