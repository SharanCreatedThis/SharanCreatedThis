...
struct SVGAssetTests {
    ...
    func physicsTable() {
        ...
        let expected = [
            ...
            // The seasonal packs. Every mass and radius here is its own — no two
            // charms in the app weigh the same or hang at the same size, which is
            // what `CharmTests` checks and what this table records.
            Expected(kind: .snowflake, mass: 2.30, radius: 0.1475, sound: .glass),
            Expected(kind: .bell, mass: 3.10, radius: 0.1530, sound: .bell),
            Expected(kind: .candyCane, mass: 2.42, radius: 0.1435, sound: .wood),
            Expected(kind: .pumpkin, mass: 3.35, radius: 0.1615, sound: .wood),
            Expected(kind: .ghost, mass: 2.15, radius: 0.1555, sound: .soft),
            Expected(kind: .bat, mass: 2.55, radius: 0.1595, sound: .soft),
            Expected(kind: .diya, mass: 3.00, radius: 0.1545, sound: .wood),
            Expected(kind: .lotus, mass: 2.50, radius: 0.1585, sound: .soft),
            Expected(kind: .lantern, mass: 2.95, radius: 0.1505, sound: .wood),
            Expected(kind: .firework, mass: 2.40, radius: 0.1565, sound: .glass),
            Expected(kind: .luckyCoin, mass: 3.20, radius: 0.1465, sound: .metal),

            // Marvel.
            Expected(kind: .spiderMan, mass: 2.96, radius: 0.1525, sound: .soft),
            Expected(kind: .captainAmericaShield, mass: 3.46, radius: 0.1515, sound: .metal),
            Expected(kind: .ironManHelmet, mass: 3.26, radius: 0.1495, sound: .metal),
            Expected(kind: .thorHammer, mass: 3.91, radius: 0.1535, sound: .metal),
            Expected(kind: .hulkFist, mass: 3.71, radius: 0.1550, sound: .wood),

            // DC.
            Expected(kind: .batmanSymbol, mass: 3.31, radius: 0.1575, sound: .metal),
            Expected(kind: .supermanShield, mass: 3.36, radius: 0.1590, sound: .metal),
            Expected(kind: .wonderWomanEmblem, mass: 3.21, radius: 0.1605, sound: .metal),
            Expected(kind: .shazamLightning, mass: 2.91, radius: 0.1485, sound: .metal),
            Expected(kind: .greenLanternRing, mass: 3.06, radius: 0.1610, sound: .glass),

            // Tamil Spiritual.
            Expected(kind: .vel, mass: 3.11, radius: 0.1620, sound: .metal),
            Expected(kind: .vinayagarCoin, mass: 3.16, radius: 0.1468, sound: .metal),
            Expected(kind: .omSymbol, mass: 2.97, radius: 0.1630, sound: .metal),
            Expected(kind: .karuppuStatue, mass: 3.61, radius: 0.1635, sound: .wood),
            Expected(kind: .templeBell, mass: 3.41, radius: 0.1645, sound: .bell),

            // BTS.
            Expected(kind: .btsMemberOne, mass: 2.65, radius: 0.1593, sound: .soft),
            Expected(kind: .btsMemberTwo, mass: 2.68, radius: 0.1601, sound: .soft),
            Expected(kind: .btsMemberThree, mass: 2.71, radius: 0.1609, sound: .soft),
            Expected(kind: .btsMemberFour, mass: 2.74, radius: 0.1617, sound: .soft),
            Expected(kind: .btsMemberFive, mass: 2.77, radius: 0.1625, sound: .soft),
            Expected(kind: .btsMemberSix, mass: 2.80, radius: 0.1633, sound: .soft),
            Expected(kind: .btsMemberSeven, mass: 2.83, radius: 0.1641, sound: .soft)
        ]
    ...
    @Test("A charm no longer has any say in what the cord is made of")
    func charmsDoNotTintTheCord() {
        // The collection's assets are drawn hanging on a gold cord, and the rope used
        // to be tinted to match. It is the rope style's job now — a user who picks
        // Silver Chain gets silver whichever charm is on the end of it — so this is
        // really a test that the old hook is gone and cannot come back by accident.
        let appearance = RopeStyle.thread.appearance
        #expect(RopeStyle.silverChain.appearance.palette != appearance.palette)
        #expect(BuiltInCharms.charm(for: .nazar).palette != appearance.palette)
    }
