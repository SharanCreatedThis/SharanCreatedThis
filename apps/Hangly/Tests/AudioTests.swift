...
struct CharmSoundTests {
    ...
    @Test("The bell sound belongs to the charms that are bells, and to no others")
    func bellsRingAndNothingElseDoes() {
        let bells = BuiltInCharms.all.filter { $0.sound == .bell }.map(\.id)

        #expect(Set(bells) == [.builtIn(.ghanta), .builtIn(.bell), .builtIn(.templeBell)])
        // Every other charm has a material, and a material is not a bell.
        #expect(BuiltInCharms.all.allSatisfy { CharmSound.allCases.contains($0.sound) })
    }
