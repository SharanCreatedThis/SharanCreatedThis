...
- **Up to three charms on one rope.** Hang one, two or three in a line down the same
  cord. Each adds its own weight, so a loaded rope hangs straighter and swings with
  more authority — and none of them can pass through another, however hard you throw
  it.
- **Five ropes, not five skins.** Thread, Leather, Gold Chain, Silver Chain and
  Neon. A gold chain swings slowly and heavily, leather is stiff and settles fast,
  neon is the lightest thing on the screen. Switch mid-swing; nothing resets.
- **Charms for the season.** Eleven more, in four packs — Halloween, Diwali,
  Christmas and New Year. The rope dresses itself when the season comes round and
  puts your own charm back when it goes. Off or pinned in one menu.
- **A rope that keeps the time.** It holds a swing about a fifth longer in the
  morning than at night, and greets you a little wider. Nothing about the speed or
  the period changes — only how long the motion lasts, which is felt rather than
  watched. Automatic, or pick an hour by hand.
- **Weather, if you want it.** Turn it on and the charm takes on the sky — warmer
  in sun, drained under cloud, wet in rain, frosted in snow, charged in a storm.
  The effects work on the shading the artwork already has, so a charm stays itself.
  Off by default, and the only thing here that uses the network.
...
- **Twenty-seven charms.** Eleven from a hand-drawn collection of protective and
  lucky charms from around the world, eleven more across four seasonal packs, and
  five geometric classics.
...
- **Genuinely cheap.** 0.6% of one core and 23 MB when settled, measured on the
  shipped build.
...
| Rope styles | Five, differing in gravity, damping, stretch ceiling and charm mass |
| Charms | One to three, each on its own node, each adding its own mass |
| Time of day | Three profiles scaling energy loss; gravity and geometry untouched |
...
- **No network calls unless you ask for one.** Weather is the only part of Hangly
  that leaves your Mac, and it is **off until you turn it on**. With it on, the
  name of a city goes to Open-Meteo every thirty minutes and a weather code comes
  back — no account, no key, no identifier, and nothing else. With it off, nothing
  is requested, no connection is opened and no timer runs.
- **No location permission.** Weather asks about the city named by your system time
  zone, or one you type. Hangly never asks macOS where you are, and the coordinates
  it sends are rounded to two decimals — about a kilometre — before they are sent
  or stored.
- **No accounts.** Nothing to sign into.

What stays on your Mac: your settings, in `~/Library/Preferences`, any charms you
import, as image files in the app's own Application Support folder, and the last
weather reading, cached beside your settings so the charm is dressed correctly
before the first request of the day. All plain files you can inspect or delete.
...
| [Weather](Docs/Weather.md) | Open-Meteo, the cache, and what the effects do |
| [Seasons](Docs/Seasons.md) | The packs, how they were drawn, and when they arrive |
