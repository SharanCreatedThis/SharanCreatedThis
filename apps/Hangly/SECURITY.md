...
Hangly runs on your Mac. It has no server, no accounts and no telemetry. It makes
one kind of network request, and only when you have turned weather on: a city name
to Open-Meteo, and a weather code back. Off by default.

What is in scope:

- Anything reachable through the weather response, which is data from outside the
  app: it is decoded into two numbers and a string, but a decoding flaw would be
  in scope.
